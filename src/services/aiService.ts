...
const AI_CONFIG = {
  baseURL:
    import.meta.env.VITE_TEXT_GENERATION_BASE_URL ||
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位专业的厨师，请根据用户提供的食材和菜系要求，生成详细的菜谱。请严格按照JSON格式返回，不要包含任何其他文字。",
        },
        {
          role: "user",
          content: prompt,
...
> => {
  try {
    // 构建提示词
    const tasteText =
      config.tastes.length > 0 ? config.tastes.join("、") : "适中";
    const sceneMap = {
      family: "家庭聚餐",
      friends: "朋友聚会",
      romantic: "浪漫晚餐",
    ...
    const styleText =
      (styleMap as Record<string, string>)[config.cuisineStyle] || "混合菜系";
    const sceneText =
      (sceneMap as Record<string, string>)[config.diningScene] || "家庭聚餐";
    const nutritionText =
      (nutritionMap as Record<string, string>)[config.nutritionFocus] ||
      "营养均衡";

    let prompt = `请为我设计一桌菜，要求如下：
- ${
      config.flexibleCount
        ? `参考菜品数量：${config.dishCount}道菜（可以根据实际情况智能调整，重点是搭配合理）`
        : `菜品数量：${config.dishCount}道菜（请严格按照这个数量生成）`
    }
- 口味偏好：${tasteText}
- 菜系风格：${styleText}
- 用餐场景：${sceneText}
- 营养搭配：${nutritionText}`;
    ...
    if (config.customDishes.length > 0) {
      prompt += `\n- ${
        config.flexibleCount ? "优先考虑的菜品" : "必须包含的菜品"
      }：${config.customDishes.join("、")}${
        config.flexibleCount
          ? "（可以作为参考，根据搭配需要决定是否全部包含）"
          : "（请确保这些菜品都包含在菜单中）"
      }`;
    }
...
智能搭配原则：
1. 菜品数量可以灵活调整（建议在${Math.max(2, config.dishCount - 2)}-${
        config.dishCount + 2
      }道之间），重点是搭配合理、营养均衡
 ...
 * @returns Promise<Recipe>
 */
export const generateDishRecipe = async (
  dishName: string,
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位专业的厨师，请根据菜品信息生成详细的制作菜谱。请严格按照JSON格式返回，不要包含任何其他文字。",
        },
        {
          role: "user",
          content: prompt,
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位专业的厨师，请根据用户提供的食材和特殊要求，生成详细的菜谱。请严格按照JSON格式返回，不要包含任何其他文字。",
        },
        {
          role: "user",
          content: prompt,
...
export const generateMultipleRecipesStream = async (
  ...
  if (completedCount < total) {
    console.warn(
      ...
      }个菜系生成失败，但已成功生成${completedCount}个菜谱`
    );
  }
};
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位专业的营养师，请根据菜谱信息生成详细的营养分析。请严格按照JSON格式返回，不要包含任何其他文字。请务必用中文回答，包括菜名也要翻译成中文",
        },
        {
          role: "user",
          content: prompt,
...
请按照以下JSON格式返回酒水搭配建议：
{
  ...
  "origin": "产地"
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位专业的侍酒师，请根据菜谱信息推荐合适的酒水搭配。请严格按照JSON格式返回，不要包含任何其他文字。",
        },
        {
          role: "user",
          content: prompt,
    ...
    return generateFallbackWinePairing(
      { id: "custom", name: recipe.cuisine } as CuisineType,
      recipe.ingredients
    );
  }
};
...
const generateFallbackNutrition = (
  ...
  const baseCalories =
    ingredients.length * 50 + Math.floor(Math.random() * 100) + 200;
  const hasVegetables = ingredients.some((ing) =>
    [
      ...
      "茄子",
      "土豆",
    ].some((veg) => ing.includes(veg))
  );
  const hasMeat = ingredients.some((ing) =>
    ["肉", "鸡", "鱼", "虾", "蛋", "牛", "猪", "羊"].some((meat) =>
      ing.includes(meat)
    )
  );
  const hasGrains = ingredients.some((ing) =>
    ["米", "面", "粉", "饭", "面条", "馒头"].some((grain) =>
      ing.includes(grain)
    )
  );
  ...
  if (!hasVegetables)
    balanceAdvice.push("建议搭配新鲜蔬菜增加维生素和膳食纤维");
  if (
    !hasMeat &&
    !ingredients.some((ing) =>
      ["豆", "蛋", "奶"].some((protein) => ing.includes(protein))
    )
  ) {
  ...
  return {
    nutrition: {
      ...
      calcium:
        hasMeat ||
        ingredients.some((ing) => ["奶", "豆"].some((ca) => ing.includes(ca)))
    ...
    healthScore:
      ...
      (hasMeat ? 1 : 0),
    balanceAdvice:
      balanceAdvice.length > 0 ? balanceAdvice : ["营养搭配合理，可以放心享用"],
...
// 生成后备酒水搭配数据
const generateFallbackWinePairing = (
  cuisine: CuisineType,
  ingredients: string[]
): WinePairing => {
  const hasSpicy = ingredients.some((ing) =>
    ["辣椒", "花椒", "胡椒", "姜", "蒜", "洋葱"].some((spice) =>
      ing.includes(spice)
    )
  );
  const hasMeat = ingredients.some((ing) =>
  ...
  const hasSeafood = ingredients.some((ing) =>
    ["鱼", "虾", "蟹", "贝", "海带", "紫菜"].some((seafood) =>
      ing.includes(seafood)
    )
  );
  const isLight = ingredients.some((ing) =>
    ["菜", "瓜", "豆腐", "蛋"].some((light) => ing.includes(light))
  );
 ...
 * @returns Promise<Recipe>
 */
export const generateDishRecipeByName = async (
  dishName: string
): Promise<Recipe> => {
  try {
 ...
 */
export const generateSauceRecipe = async (
  sauceName: string
): Promise<SauceRecipe> => {
  try {
...
请按照以下JSON格式返回酱料配方：
{
  ...
  "description": "酱料特色描述"
    ...
    throw new Error(
      `酱料大师表示"${sauceName}"这个配方太有挑战性了，请稍后重试`
    );
  }
};
 ...
 */
export const recommendSauces = async (
  ...
  try {
    ...
    const useCaseText = preferences.useCase
      .map((uc) => (useCaseMap as Record<string, string>)[uc] || uc)
      .join("、");
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位专业的酱料推荐专家，能够根据用户的口味偏好和使用场景推荐最合适的酱料。请严格按照JSON格式返回，不要包含任何其他文字。请务必用中文回答。",
        },
        {
          role: "user",
          content: prompt,
 ...
 * @returns Promise<SauceRecipe>
 */
export const createCustomSauce = async (
  request: CustomSauceRequest
): Promise<SauceRecipe> => {
  try {
    const baseTypeMap = {
      oil: "油性酱料",
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位富有创意的酱料创作大师，擅长根据用户需求创作独特的酱料配方。请严格按照JSON格式返回，不要包含任何其他文字。请务必用中文回答。",
        },
        {
          role: "user",
          content: prompt,
 ...
 * @returns Promise<string[]>
 */
export const getSaucePairings = async (
  sauceName: string
): Promise<string[]> => {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "你是一位专业的美食搭配专家，精通各种酱料与菜品的最佳搭配方法。请严格按照JSON格式返回，不要包含任何其他文字。请务必用中文回答。",
        },
        {
          role: "user",
          content: prompt,
 ...
 */
export const generateDailyFortune = async (
...
请按照以下JSON格式返回占卜结果：
{
  ...
  "steps": ["制作步骤1", "制作步骤2"]
    ...
    const fortune: FortuneResult = {
      ...
      mysticalMessage:
        fortuneData.mysticalMessage || "命运之轮正在转动，美味即将降临...",
 ...
 * @returns Promise<FortuneResult>
 */
export const generateMoodCooking = async (
  params: MoodFortuneParams
): Promise<FortuneResult> => {
  try {
    const moodText = params.moods.join("、");
    const intensityText = ["很轻微", "轻微", "一般", "强烈", "非常强烈"][
      params.intensity - 1
    ];
 ...
 * @returns Promise<FortuneResult>
 */
export const generateCoupleCooking = async (
  params: CoupleFortuneParams
): Promise<FortuneResult> => {
  try {
...
请按照以下JSON格式返回占卜结果：
{
  ...
  "steps": ["合作步骤1", "合作步骤2"]
    ...
    const fortune: FortuneResult = {
      ...
      mysticalMessage:
        fortuneData.mysticalMessage || "缘分天注定，美食见真情...",
      ingredients: fortuneData.ingredients || [],
      steps: fortuneData.steps || [],
    };
 ...
 * @returns Promise<FortuneResult>
 */
export const generateNumberFortune = async (
  params: NumberFortuneParams
): Promise<FortuneResult> => {
  try {
...
请按照以下JSON格式返回占卜结果：
{
  ...
  "steps": ["制作步骤1", "制作步骤2"]
    ...
    const fortune: FortuneResult = {
      ...
      mysticalMessage:
        fortuneData.mysticalMessage || "数字是宇宙的语言，美食是心灵的慰藉...",
      ingredients: fortuneData.ingredients || [],
      steps: fortuneData.steps || [],
    };
