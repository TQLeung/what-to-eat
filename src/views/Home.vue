              ...
              >
                ...
                >
                  ...
                  <span class="font-medium"
                    >已选择 {{ ingredients.length }}/10</span
                  >
                </div>
              </div>
            </div>
          ...
          >
            <div>
              ...
              <div
                ...
                class="mb-4 p-3 bg-blue-50 border-2 border-blue-200 rounded-lg text-center"
              >
                <p class="text-sm text-blue-700 mb-2">
                  ✓ 已设置自定义要求，将优先使用自定义要求生成菜谱
                </p>
                <button
              ...
              <div class="border-t border-gray-200 pt-4">
                ...
                <div
                  ...
                  class="bg-blue-100 border-2 border-blue-300 rounded-lg p-3"
                >
                  <!-- 快速预设选项 -->
                  <div class="mb-3">
                    <button
                  ...
                  <div class="mt-2 pt-2 border-t border-blue-200">
                    ...
                    >
                      <span>💡 提示：越具体越好！</span>
                      <span
                        :class="{ 'text-red-500': customPrompt.length > 180 }"
                        >{{ customPrompt.length }}/200</span
                      >
                    </div>
                  </div>
                </div>
          ...
          >
            <div class="text-center h-full flex flex-col">
              ...
              <div class="bg-gray-50 rounded-lg p-3 mb-4 text-left flex-1">
                <h3
                  class="font-bold text-sm text-gray-700 mb-2 flex items-center gap-2"
                >
                  <span>📋</span>
                  <span>当前配置</span>
                </h3>
                ...
                <div class="mb-2">
                  ...
                  >
                    <span
                      v-for="ingredient in ingredients"
                      :key="ingredient"
                      class="inline-block bg-yellow-200 text-yellow-800 px-2 py-1 rounded text-xs"
                    >
                      {{ ingredient }}
                    </span>
                  </div>
                ...
                <div class="mb-2">
                  ...
                  >
                  <div
                    v-if="selectedCuisines.length > 0 && !customPrompt.trim()"
                    class="mt-1"
                  >
                    <div
              ...
              <!-- 提示信息 -->
              <div class="text-sm">
                <p v-if="customPrompt.trim()" class="text-blue-600">
                  🎯 将根据您的自定义要求生成菜谱
                </p>
                <p
                  v-else-if="selectedCuisines.length > 0"
      ...
      <div ref="resultsSection" class="mt-16">
        ...
        >
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              >
                ...
                <div v-else-if="cuisineInfo.error" class="bg-white error-card">
                  <!-- 错误头部 -->
                  <div
                    class="bg-gradient-to-r from-red-400 to-orange-400 text-white p-4 md:p-6 border-b-2 border-black"
                  >
                  ...
                  <div class="p-4 md:p-6 text-center">
                    <div class="mb-4">
                      ...
                      <p class="text-gray-600 text-sm mb-4">
                        {{
                          cuisineInfo.name
                        }}看了看你的食材，挠了挠头说："这个组合我还没学会呢！"
                      </p>
                    </div>

                    <!-- 建议区域 -->
                    <div
                      class="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-4 mb-4"
                    >
                      <h5
                        class="text-sm font-bold text-yellow-800 mb-2 flex items-center gap-1 justify-center"
                      >
                        💡 大师的建议
                      </h5>
                ...
                <div v-else class="bg-white loading-card">
                  <!-- 菜系头部 -->
                  <div
                    class="bg-gradient-to-r from-gray-400 to-gray-500 text-white p-4 md:p-6 border-b-2 border-black"
                  >
                    <div class="flex items-center justify-between">
                      <div class="flex-1">
                        <h3
                    ...
                    <div class="mb-4">
                      ...
                      <div class="space-y-2">
                        <div
                          v-for="i in 3"
                          :key="i"
                          class="flex gap-2 p-2 bg-gray-50 rounded border border-gray-200"
                        >
                          <div
                            class="flex-shrink-0 w-5 h-5 bg-gray-300 rounded shimmer-effect"
                          ></div>
                          <div class="flex-1 space-y-1">
                            <div
                              class="h-3 bg-gray-300 rounded shimmer-effect"
                              :style="{ width: 60 + Math.random() * 30 + '%' }"
                            ></div>
                            <div
                              class="h-2 bg-gray-200 rounded shimmer-effect"
          ...
          <div v-else-if="errorMessage" class="text-center py-12">
            <div
              class="w-16 h-16 bg-red-100 rounded-lg flex items-center justify-center mx-auto mb-4"
            >
          ...
          <div v-else-if="recipes.length === 0" class="text-center py-12">
            <div
              class="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center mx-auto mb-4"
            >
          ...
          <div
            v-else-if="recipes.length > 0 && cuisineSlots.length === 0"
...
const scenePresets = [
  {
    id: "family",
    name: "家庭聚餐",
    prompt: "适合全家人一起享用的家常菜，老少皆宜，营养均衡",
  },
  {
    id: "romantic",
    name: "浪漫晚餐",
    prompt: "适合情侣约会的精致菜品，卖相精美，氛围浪漫",
  },
  {
    id: "quick",
    name: "快手菜",
    prompt: "制作简单快速的菜品，15分钟内完成，适合忙碌时光",
  },
  {
    id: "party",
    name: "聚会小食",
    prompt: "适合朋友聚会的下酒菜或小食，方便分享",
  },
  {
    id: "breakfast",
    name: "营养早餐",
    prompt: "健康营养的早餐搭配，提供一天的活力",
  },
  {
    id: "lunch",
    name: "工作午餐",
    prompt: "适合上班族的午餐，营养丰富且不油腻",
  },
];
...
const addIngredient = () => {
  ...
  if (
    ingredient &&
    !ingredients.value.includes(ingredient) &&
    ingredients.value.length < 10
  ) {
...
const generateRecipes = async () => {
  ...
  if (customPrompt.value.trim()) {
    ...
    // 使用菜系生成菜谱 - 立即初始化菜系槽位
    let selectedCuisineObjects = cuisines.filter((c) =>
      selectedCuisines.value.includes(c.id)
    );
  ...
  try {
    if (customPrompt.value.trim()) {
      ...
      const progressInterval = setInterval(() => {
        if (cuisineSlots.value[0] && !cuisineSlots.value[0].recipe) {
          cuisineSlots.value[0].progress = Math.min(
            cuisineSlots.value[0].progress + Math.random() * 15,
            90
          );
        }
      }, 500);
      ...
      await generateMultipleRecipesStream(
        ...
        (recipe: Recipe, index: number, total: number) => {
          ...
          const targetSlot = cuisineSlots.value.find(
            (slot) =>
              selectedCuisineObjects[index] &&
              slot.id === selectedCuisineObjects[index].id
          );
          ...
          const completedCount =
            ...
            cuisineSlots.value.filter((slot) => slot.error).length;
    ...
    // 显示错误信息
    errorMessage.value =
      error instanceof Error ? error.message : "AI生成菜谱失败，请稍后重试";
  } finally {
    // 确保加载状态被清除
    if (isLoading.value) {
      isLoading.value = false;
...
const retryFailedCuisine = async (failedSlot: CuisineSlot) => {
  ...
  const progressInterval = setInterval(() => {
    if (!failedSlot.recipe && !failedSlot.error) {
      failedSlot.progress = Math.min(
        failedSlot.progress + Math.random() * 10,
        85
      );
    }
  }, 500);
  ...
  try {
    ...
    failedSlot.errorMessage =
      error instanceof Error ? error.message : `${cuisine.name}还是不会这道菜`;
    failedSlot.progress = 0;
